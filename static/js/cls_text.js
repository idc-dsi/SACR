...
function plaintext(s) {
   return s.replace(/{[^:\s]+(:[^=\s]+=([^"\s]*|"[^"]*")(,[^=\s]+=([^"\s]*|"[^"]*"))*)\s+/g, "").replace(/}/g, "").replace(/^#.*$/gm, "");
}

function rtlScore(s) {
   return (s.match(/\p{Script=Hebrew}|\p{Script=Arabic}/gu) || []).length / (s.match(/\p{Alphabetic}/gu) || []).length;
}
...
class Text {
   ...
   constructor(autocomplete) {
      ...
      this.makeTable();
      document.getElementById("textContainer").appendChild(this.div);
   ...
   makeTable() {
      ...
      var popupContainer = document.createElement("td");
      popupContainer.id = "popupContainer";
      popupContainer.style.display = "none";

      row.appendChild(textContainer);
      row.appendChild(popupContainer);
   ...
   parse() {
      ...
      var parser = new SacrParser(this.div, this.raw_text,
         this.tokenizationType, false);

      parser.parseText();

      if (rtlScore(plaintext(this.raw_text)) > 0.5) {
   ...
   init(dataLoader = null) {
      ...
      if (dataLoader) {
         ...
         this.dataLoader = new DataLoader(callback, this.autocomplete);
      }
   }
   ...
   markChanged(changed = true) {
      this.text_prev.push(this.computeText());
      Array.prototype.forEach.call(
         this.div.getElementsByClassName("comment"),
         (x) => x.classList.toggle("saved", !changed)
      );
   };
   toggleFinished() {
   ...
   computeText() {
      return new Exporter().computeText(true);
   };
